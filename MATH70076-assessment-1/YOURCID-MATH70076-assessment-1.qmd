...
```
We first examine the validity of the proposal: “the distribution of river flow values is constant over time”.  The proposal suggests the river flow’s statistical behaviour, for example, the mean, variance, and shape of the distribution do not change across years. We then conduct the box plot and the density plot to examine whether there is a long-term trend in the mean or variability of the river flow’s values. 
```{r}
#| echo: false
#| fig-width: 5
#| fig-height: 3
...
We then draw the ACF plots (@fig-exceed-2 & @fig-exceed-3) to investigate the independence of exceedances. The ACF plots conduct autocorrelation diagnostics of time series and capture patterns of temporal dependence. @(references). 
