...
```{r}
# YOUR CODE GOES HERE
#' Quantile function of the Generalized Pareto Distribution (GPD)
#'
#' Computes quantiles (inverse CDF) for the GPD with given threshold (u), scale (sigma),
#' and shape (xi) parameters. The function works similarly to qnorm() or qunif().
#'
#' @param p Numeric vector of probabilities, 0 <= p < 1.
#' @param u Numeric scalar, threshold (location) parameter.
#' @param sigma Positive numeric scalar, scale parameter.
#' @param xi Numeric scalar, shape parameter.
#'
...
#'
#' @export
qgpd <- function(p, u = 0, sigma = 1, xi = 0) {
  # input checks
  stopifnot(
  is.numeric(xi),
  length(xi) == 1,
...
sigma_val <- 2
xi_val <- -0.4
p_vec <- c(0.5, 0.75, 0.99)

# use qgpd function
x_p <- qgpd(p_vec, u = u_val, sigma = sigma_val, xi = xi_val)
x_p
```
