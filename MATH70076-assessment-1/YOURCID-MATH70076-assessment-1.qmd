...
#| eval: false
#| echo: false
install.packages(c("rmarkdown", "knitr", "kableExtra", "ggplot2", "stats", "ADGofTest", "KScorrect"))
```
...
```
We first examine the validity of the proposal: “the distribution of river flow values is constant over time”.  The proposal suggests the river flow’s statistical behaviour, for example, the mean, variance, and shape of the distribution do not change across years. We then conduct the box plot and the density plot using "ggplot2"  [@Ross2018] to examine whether there is a long-term trend in the mean or variability of the river flow’s values. 
```{r}
#| echo: false
#| fig-width: 5
#| fig-height: 3
#| label: fig-boxplot
#| fig-cap: "Boxplots to examine the yearly distribution of baseline mean."
## Figure 1 (Boxplot)
...
ggplot(riverflow, aes(x = Year, y = baseline_mean), cex.lab = 1.4) +
  geom_boxplot(fill = "lightblue", color = "black") +
  theme_light() +
  labs(title = "Yearly Distribution of Baseline Mean",
       y = "Baseline Mean of River Flow (m³/s)", x = "Year") 
```
The boxplot @fig-boxplot reveals the stability of central tendency and spread. We can see a clear long-term increase in flow magnitudes according to the baseline mean of river flow. Additionally, the yearly medians were not stable with a continuous increase in the value, suggesting strong evidence against hydrologist’s first proposal. The interquartile ranges are relatively consistent, which gives evidence for constant variance for the baseline means of river flow. From the gradual increase in the yearly median of river flow mean, we may conclude that the proposal was not valid for the dataset provided. 
```{r}
#| echo: false
#| fig-width: 5
#| fig-height: 3
#| label: fig-density
#| fig-cap: "Density plot to witness the shape of river flow each year."
...
ggplot(riverflow, aes(x = flow, colour = Year), cex.lab =1.4) +
  ...
  theme_light() +
  labs(x = "Flow (m³/s)", y = "Density",
       title = "Kernel Density Estimates by Year")
```
@fig-density on the next page shows the kernel density estimates of river flow for each year between 2015 and 2024. The kernel density in density plots helps us to estimate the probability density function and captures the underlying patterns without assuming a specific distribution [@Tollefson2021]. It illustrates the overall shape of the flow distribution for each year and informs stability in higher-order distributional properties, like skewness, tail behaviour. The distributions across years exhibit a broadly similar shape with right skewness. However, there are distinct differences in the height and spread of the peaks, which indicate year-to-year variations. A systematic trend is observed in the figure, as the flow values become less right-skewed from 2015 to 2024, and the peaks of the flows shift rightwards as the year increases. Hence, this density plot does not support the proposal which reckons the distribution of flow values is constant over time.

We then examine the validity of the second proposal: “for river flows exceeding 75 cumecs, it is appropriate to model these data as independent and identically distributed GPD(sigma = 29.7, xi = 0.62, u = 0). The proposal suggests that for flows larger than 75 cumecs, the exceedances above the threshold follow a Generalised Pareto Distribution (GPD) with those specified shape and scale parameters, and these exceedances have no serial correlation with each other and follow the same GPD distribution across time. 
```{r}
#| echo: false
#| fig-width: 6
#| fig-height: 4
#| label: fig-qqplot2
#| fig-cap: "QQ plot to examine the GPD's goodness of fit."
## Figure 3 (QQ plot)
u <- 75
...
legend("topleft",
       ...
       bty = "n")
```
For values exceeding 75 cumecs, we extract the exceedances and compare them against a theoretical GPD(with sigma=29.7, xi=0.62). In the QQ plot @fig-qqplot2, the sample quantiles of exceedances were plotted against theoretical quantiles computed from the specified GPD model. Although there are slight deviations seen in the upper tail, it supports the proposal as the points scatter closely to the 45° reference line. Overall, the proposed GPD model appears reasonably fit to the tail behaviour of the river flow data. 
```{r}
#| echo: false
#| layout-ncol: 3
#| layout-nrow: 1
...
#| fig-height: 4
#| label: fig-exceed
#| fig-cap: "Trace plot & ACF plots to examine the independence of river flow."
#| fig-subcap:
#|   - "Trace plot of flow > 75 cumecs"
#|   - "ACF plot of All River Flow"
#|   - "ACF plot of Flow Exceeding 75 cumecs"
## Figure 4 (Trace plot & ACF plots)
plot(exceed, type = "l", col = "black",
     xlab = "Date", ylab = "River Flow (m³/s)",
     main = "Time Series of River Flow")

acf(as.numeric(riverflow$flow), main = "ACF of All River Flow")
acf(exceed, main = "ACF of Exceedance Magnitudes")
```
Focusing on the “independent and identically distributed (i.i.d.)” component, we first draw a trace plot (@fig-exceed-1) of the exceedances. There is a lack of systematic trends and substantial clustering except for one peak point, indicating minimal temporal dependence in the values and supporting the i.i.d. assumption.

We then draw the ACF plots (@fig-exceed-2 & @fig-exceed-3) to investigate the independence of exceedances. The ACF plots conduct autocorrelation diagnostics of time series and capture patterns of temporal dependence [@Tollefson2021]. 
There is no bar extending beyond 95% confidence bounds, which means that the exceedance magnitudes show no significant autocorrelation problems. Besides, no geometrical and gradual decay in bar heights shows that there is no long-run dependency in the values. Furthermore, there are no regular spikes at specified lags. This supports our conclusion from the trace plot, which says there is no seasonal pattern among the exceedances, suggesting that the river flow exceeding 75 can be treated as independent and identically distributed. To summarise, these three visualisations in one figure (@fig-exceed) give strong support to the hydrologist’s second proposal.
