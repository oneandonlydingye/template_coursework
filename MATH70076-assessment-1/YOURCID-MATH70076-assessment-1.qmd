...
install.packages(c("rmarkdown", "knitr", "kableExtra", 
                   "ggplot2", "stats", "ADGofTest", "KScorrect"))
```
...
#| fig-height: 5
#| label: fig-exceed
#| fig-cap: "Trace plot & ACF plots to examine the i.i.d. assumptions of flow."
#| fig-subcap:
...
```
Focusing on the “independent and identically distributed (i.i.d.)” component, we produce three visualisations in one figure (@fig-exceed) to validate the hydrologist’s second proposal.We first draw a trace plot (@fig-exceed-1) of the exceedances. There is a lack of systematic trends and substantial clustering except for one peak point, indicating minimal temporal dependence in the values and supporting the i.i.d. assumption.

We then utilise the ACF plot (@fig-exceed-2) to investigate the independence of exceedances. The ACF plots conduct autocorrelation diagnostics of time series and capture patterns of temporal dependence [@Tollefson2021]. There is no bar extending beyond 95% confidence bounds in @fig-exceed-3, which means that the exceedance magnitudes show no significant autocorrelation problems. Besides, no geometrical and gradual decay in bar heights shows that there is no long-run dependency in the values. Furthermore, there are no regular spikes at specified lags. This supports our conclusion from the trace plot, which says there is no seasonal pattern among the exceedances, suggesting that the river flow exceeding 75 can be treated as independent and identically distributed. To summarise, @fig-exceed-1 and @fig-exceed-2 give strong support to the hydrologist’s second proposal. 

Lastly, the ACF plot can also be used to examine al river flow's values. With some notable spikes above confidence bounds and a gradual decline in the bar's heights, the @fig-exceed-3 supports our conclusion towards the first proposal of the hydrologist which disagrees her idea of "the distribution of river flow values is constant over time".
