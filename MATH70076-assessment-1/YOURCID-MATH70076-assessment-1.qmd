...
format:
  ...
  pdf: default
format-links: false
bibliography: "MSc_Statistics_coursework_quarto.bib"
---
...
The inverse CDF function could be divided into 2 situations ($\xi \ne 0$ and $\xi = 0$). We first handle the case for $\xi \ne 0$.

If $$\left(1 + \frac{\xi (x - u)}{\sigma}\right)_+ = \max\left(1 + \frac{\xi (x - u)}{\sigma}, 0\right) = 1 + \frac{\xi (x - u)}{\sigma}, $$

$$
1 + \frac{\xi (x - u)}{\sigma} > 0 \quad \iff
$$
$$
\frac{\xi (x - u)}{\sigma} > -1 \quad \iff
$$
$$
\xi (x - u) > -\sigma \quad \iff
$$
$$
x - u > -\sigma / \xi \quad \iff
$$

As $\sigma > 0$, we have 2 situations according to the value of $\xi$:
$$
x
\begin{cases}
> u - \sigma / \xi, & if \quad \xi > 0 \\
< u - \sigma / \xi, & if \quad \xi < 0
\end{cases}
$$

When $\xi > 0$, the inequality holds as $x \ge u$.  

When $\xi < 0$, the distribution reaches the finite upper endpoint of $x^+ = u - \sigma / \xi$, then $\left(1 + \dfrac{\xi (x - u)}{\sigma}\right)_+ = 0$. Therefore, we get:
$$
F^{-1}(1) = x^+ = 
\begin{cases}
u - \sigma / \xi &  if \quad \xi < 0 \\
+\infty & if \quad \xi \ge 0
\end{cases}
$$

We then compute of quantile function from the CDF of the GPD when $(\xi \neq 0)$:
$$
...
As a result, the quantile function $F_X^{-1} : [0,1] \rightarrow [u, x^+]$ of $X \sim \text{GPD}(u, \sigma, \xi)$ can be written as:

$$
F_X^{-1}(A) = 
\begin{cases}
u + \frac{\sigma}{\xi}\left[ (1 - A)^{-\xi} - 1 \right], & for \quad A\in [0,1), \quad \xi \ne 0 \\[8pt]
u - \sigma \ln(1 - A), & for \quad A\in [0,1) , \quad \xi = 0
\end{cases}
$$
with $F_X^{-1}(1) = x^+$.
...
for (a in datasets) {
  ...
  legend("topleft",
       legend = c("recorded parameters", "simulated parameters", "reference line"),
       pch = c(16, 17, NA), lty = c(NA, NA, 2), col = c("black", "blue","red"), bty = "n", cex = 2)
}
```
...
ggplot(riverflow, aes(x = Year, y = flow), cex.lab = 1.2) +
  geom_boxplot(fill = "lightblue", color = "black") +
  theme_light() +
  labs(title = "Yearly Distribution of River Flow",
       y = "Baseline Mean of River Flow (m³/s)", x = "Year") +
       coord_cartesian(ylim = c(0, 150))
```
The boxplot @fig-boxplot demonstrates a distribution of yearly river flow values and helps us to examine the stability of central tendency and spread of data. We can see a clear long-term increase in flow magnitudes over the years. Additionally, the median flow values show a slight continuous upward trend in the value, which provides strong evidence against hydrologist’s assumption of a stationary mean. The interquartile ranges are relatively stable from 2015 to 2019, but has increased since 2022, which gives evidence for inconsistent variability of flow data. Overall, the figure indicates that while the spread of flow values does not change markedly, their central tendency has increased over the years, which contradicts the hydrologist’s first proposal.
```{r}
#| echo: false
#| fig-width: 5
#| fig-height: 2.8
...
```
To validate the “independent and identically distributed (i.i.d.)” assumption, we produce three visualisations in one figure (@fig-exceed). The time series plot (@fig-exceed-1) displays exceedances over time and demonstrates no evident systematic trends and volatility clustering patterns except for one isolated peak point of river flow. This indicates the distribution of exceedances is relatively stable and particularly supports the "identically distributed" assumption.
